...
describe.skipIf(!hasGroq)("Groq API", () => {
  it.each([
    ["openai/gpt-oss-120b", "Groq-GPT-OSS-120B (1st fallback)"],
    ["qwen/qwen3.6-27b", "Groq-Qwen-27B (2nd fallback)"],
    ["openai/gpt-oss-20b", "Groq-GPT-OSS-20B (3rd/final fallback)"],
  ])("%s responds to a simple prompt (%s)", async (model) => {
